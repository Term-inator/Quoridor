"""Command-line entry point for the bounded PPO validation."""
...
from experiments.ppo.experiment import run_experiment, run_smoke
...
def main() -> None:
    ...
    parser.add_argument(
        "--results-dir",
        type=Path,
        default=Path("experiments/ppo/results/seed-0"),
    )
    parser.add_argument(
        "--artifacts-dir",
        type=Path,
        default=Path("experiments/ppo/artifacts/seed-0"),
    )
    ...
    parser.add_argument("--final-games", type=int, default=1000)
    arguments = parser.parse_args()
    device = _device(arguments.device)

    if arguments.smoke:
        artifacts = run_smoke(arguments.results_dir, device=device)
    else:
