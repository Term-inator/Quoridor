...
from experiments.dqn.experiment import (
    recover_user_stopped_experiment,
    run_experiment,
    run_smoke,
)
...
def main() -> None:
    ...
    parser.add_argument(
        "--results-dir",
        type=Path,
        default=Path("experiments/dqn/results/seed-0"),
    )
    parser.add_argument(
        "--artifacts-dir",
        type=Path,
        default=Path("experiments/dqn/artifacts/seed-0"),
    )
    parser.add_argument(
        "--smoke-dir",
        type=Path,
        default=Path("experiments/dqn/artifacts/smoke"),
    )
    ...
    if arguments.smoke and arguments.recover_stopped:
        parser.error("--smoke and --recover-stopped are mutually exclusive")
    if arguments.smoke:
        artifacts = run_smoke(arguments.smoke_dir, device=device)
    elif arguments.recover_stopped:
        artifacts = recover_user_stopped_experiment(
